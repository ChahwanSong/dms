...
suffix="${DMS_PHASE5_DB_SUFFIX:-$(date +%Y%m%d%H%M%S)}"
operational_db="${DMS_PHASE5_OPERATIONAL_DB:-dms_phase5_${suffix}}"
observability_db="${DMS_PHASE5_OBSERVABILITY_DB:-dms_phase5_obs_${suffix}}"
postgres_host="${DMS_PHASE5_POSTGRES_HOST:-192.168.56.11}"
postgres_port="${DMS_PHASE5_POSTGRES_PORT:-30432}"
postgres_user="${DMS_PHASE5_POSTGRES_USER:-appuser}"
...
export PHASE5_OPERATIONAL_DB="${operational_db}"
export PHASE5_OBSERVABILITY_DB="${observability_db}"
export PHASE5_POSTGRES_HOST="${postgres_host}"
export PHASE5_POSTGRES_PORT="${postgres_port}"
export PHASE5_POSTGRES_USER="${postgres_user}"
...
db_names = [os.environ["PHASE5_OPERATIONAL_DB"], os.environ["PHASE5_OBSERVABILITY_DB"]]
for db_name in db_names:
    if not re.match(r"^[A-Za-z_][A-Za-z0-9_]*$", db_name):
        raise SystemExit(f"unsafe database name: {db_name}")

with psycopg.connect(
    host=os.environ["PHASE5_POSTGRES_HOST"],
    port=int(os.environ["PHASE5_POSTGRES_PORT"]),
    dbname="postgres",
    user=os.environ["PHASE5_POSTGRES_USER"],
...
cd "${repo_dir}"
"${python_bin}" scripts/phase5_kubernetes_quota_lifecycle.py
