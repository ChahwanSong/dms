...
suffix="${DMS_PHASE4_DB_SUFFIX:-$(date +%Y%m%d%H%M%S)}"
operational_db="${DMS_PHASE4_OPERATIONAL_DB:-dms_phase4_${suffix}}"
observability_db="${DMS_PHASE4_OBSERVABILITY_DB:-dms_phase4_obs_${suffix}}"
postgres_host="${DMS_PHASE4_POSTGRES_HOST:-192.168.56.11}"
postgres_port="${DMS_PHASE4_POSTGRES_PORT:-30432}"
postgres_user="${DMS_PHASE4_POSTGRES_USER:-appuser}"
...
export PHASE4_OPERATIONAL_DB="${operational_db}"
export PHASE4_OBSERVABILITY_DB="${observability_db}"
export PHASE4_POSTGRES_HOST="${postgres_host}"
export PHASE4_POSTGRES_PORT="${postgres_port}"
export PHASE4_POSTGRES_USER="${postgres_user}"
...
db_names = [os.environ["PHASE4_OPERATIONAL_DB"], os.environ["PHASE4_OBSERVABILITY_DB"]]
for db_name in db_names:
    if not re.match(r"^[A-Za-z_][A-Za-z0-9_]*$", db_name):
        raise SystemExit(f"unsafe database name: {db_name}")

with psycopg.connect(
    host=os.environ["PHASE4_POSTGRES_HOST"],
    port=int(os.environ["PHASE4_POSTGRES_PORT"]),
    dbname="postgres",
    user=os.environ["PHASE4_POSTGRES_USER"],
...
cd "${repo_dir}"
"${python_bin}" scripts/phase4_kubernetes_quota_live.py
