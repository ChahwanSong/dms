...
suffix="${DMS_PHASE7_DB_SUFFIX:-$(date +%Y%m%d%H%M%S)}"
operational_db="${DMS_PHASE7_OPERATIONAL_DB:-dms_phase7_${suffix}}"
observability_db="${DMS_PHASE7_OBSERVABILITY_DB:-dms_phase7_obs_${suffix}}"
postgres_host="${DMS_PHASE7_POSTGRES_HOST:-192.168.56.11}"
postgres_port="${DMS_PHASE7_POSTGRES_PORT:-30432}"
postgres_user="${DMS_PHASE7_POSTGRES_USER:-appuser}"
...
export PHASE7_OPERATIONAL_DB="${operational_db}"
export PHASE7_OBSERVABILITY_DB="${observability_db}"
export PHASE7_POSTGRES_HOST="${postgres_host}"
export PHASE7_POSTGRES_PORT="${postgres_port}"
export PHASE7_POSTGRES_USER="${postgres_user}"
...
db_names = [os.environ["PHASE7_OPERATIONAL_DB"], os.environ["PHASE7_OBSERVABILITY_DB"]]
for db_name in db_names:
    if not re.match(r"^[A-Za-z_][A-Za-z0-9_]*$", db_name):
        raise SystemExit(f"unsafe database name: {db_name}")

with psycopg.connect(
    host=os.environ["PHASE7_POSTGRES_HOST"],
    port=int(os.environ["PHASE7_POSTGRES_PORT"]),
    dbname="postgres",
    user=os.environ["PHASE7_POSTGRES_USER"],
...
cd "${repo_dir}"
"${python_bin}" scripts/phase7_operational_query_and_block_update.py
