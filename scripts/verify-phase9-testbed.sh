...
suffix="${DMS_PHASE9_DB_SUFFIX:-$(date +%Y%m%d%H%M%S)}"
operational_db="${DMS_PHASE9_OPERATIONAL_DB:-dms_phase9_${suffix}}"
observability_db="${DMS_PHASE9_OBSERVABILITY_DB:-dms_phase9_obs_${suffix}}"
postgres_host="${DMS_PHASE9_POSTGRES_HOST:-192.168.56.11}"
postgres_port="${DMS_PHASE9_POSTGRES_PORT:-30432}"
postgres_user="${DMS_PHASE9_POSTGRES_USER:-appuser}"
python_bin="${DMS_PYTHON:-}"
namespace="${DMS_PHASE9_NAMESPACE:-dms-phase9}"
node_port="${DMS_PHASE9_API_NODE_PORT:-30089}"
auth_token="${DMS_PHASE9_AUTH_TOKEN:-phase9-testbed-token}"
registry_host="${DMS_PHASE9_REGISTRY_HOST:-192.168.56.11:5000}"
k8s_image="${DMS_PHASE9_K8S_IMAGE:-testbed-registry:5000/dms:phase9}"
docker_image="${DMS_PHASE9_DOCKER_IMAGE:-${registry_host}/dms:phase9}"
manifest_dir="${shared_dir}/dms-phase9-${suffix}"
...
export PHASE9_OPERATIONAL_DB="${operational_db}"
export PHASE9_OBSERVABILITY_DB="${observability_db}"
export PHASE9_POSTGRES_HOST="${postgres_host}"
export PHASE9_POSTGRES_PORT="${postgres_port}"
export PHASE9_POSTGRES_USER="${postgres_user}"
...
db_names = [os.environ["PHASE9_OPERATIONAL_DB"], os.environ["PHASE9_OBSERVABILITY_DB"]]
for db_name in db_names:
    if not re.match(r"^[A-Za-z_][A-Za-z0-9_]*$", db_name):
        raise SystemExit(f"unsafe database name: {db_name}")

with psycopg.connect(
    host=os.environ["PHASE9_POSTGRES_HOST"],
    port=int(os.environ["PHASE9_POSTGRES_PORT"]),
    dbname="postgres",
    user=os.environ["PHASE9_POSTGRES_USER"],
...
if [[ "${DMS_PHASE9_SKIP_IMAGE_BUILD:-0}" != "1" ]]; then
  ...
  if ! docker push "${docker_image}"; then
    ...
    image_archive="${manifest_dir}/dms-phase9-image.tar"
    docker save "${docker_image}" -o "${image_archive}"
    ssh c1-control \
      "skopeo copy --dest-tls-verify=false docker-archive:/shared_directory/$(basename "${manifest_dir}")/dms-phase9-image.tar docker://192.168.56.11:5000/dms:phase9"
  fi
fi
...
cd "${repo_dir}"
DMS_AUTH_SHARED_TOKEN="" "${python_bin}" scripts/phase9_kubernetes_quota_operational_hardening.py

if [[ "${DMS_PHASE9_CLEANUP:-1}" == "1" ]]; then
  printf '== Cleanup Phase 9 manifests ==\n'
