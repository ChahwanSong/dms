...
import { parsePathsCsv, type ScanRow } from "./helpers";

// CSV/text popup for scan batch requests. Two modes:
...
export default function ScanCsvModal({
  title,
  hint,
  initialText,
  mode,
  ...
  function replace() {
    ...
    if (!parsed.rows.length) {
      setErr("내용에 항목이 없습니다 (한 줄에 스캔 경로 하나).");
      return;
    }
    setErr(null);
      ...
      <div className="modal wide" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="form">
          ...
          <textarea
            ...
            spellCheck={false}
            rows={12}
            placeholder={mode === "replace" ? "스캔 경로   (한 줄에 한 항목)" : undefined}
            onChange={(e) => {
