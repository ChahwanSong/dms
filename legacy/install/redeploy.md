...
|---|---|---|
| `src/dms/` (api·planner·workers·adapters·query·repositories 등) | `dms` | ns `dms` Deployment 6개: `dms-api`·`dms-api-internal`·`dms-planner`·`dms-dm-worker`·`dms-retention`·`dms-sanity-reconciler` |
...
kubectl -n dms set image deploy/dms-api-internal      api=$NEW   # 내부 평면(agent + 포탈 BFF), 동일 dms 이미지
...
> **누락 주의**: 하나라도 빠뜨리면 그 워크로드만 옛 이미지로 남아 옛 동작을 계속한다. 테스트베드는
> Deployment가 늘 수 있으니 위 `grep /dms:` 결과와 목록이 일치하는지 확인한다.
...
# 검증: dms/db 연결 OK. 파드에서 직접 봐야 한다 — ingress-nginx가 /healthz를 선점한다(dms-06 §5)
...
## 4. 이전 릴리스(RM 포함)에서 올라오는 경우

RM(Resource Management)이 아직 있던 릴리스에서 처음 올라온다면 **일회성 정리**가 필요하다 —
워크로드 삭제, DB 정리 SQL, 스토리지 매핑 재검사. 절차는
[`migration-rm-removal.md`](migration-rm-removal.md)에 있다. 이미 올린 배포는 건너뛴다.
