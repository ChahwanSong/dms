...
// ~6개만 남긴다(24h 시간 버킷 24개를 다 쓰면 겹쳐서 아무것도 못 읽는다). 저밀도
// (≤SPARSE_MAX)는 열마다 라벨을 직접 달아 이 함수를 안 쓴다 -- 실제 호출은 11버킷
// 이상뿐이라 ≤8 가지는 이제 순수 함수 계약으로만 남아 있다.
export function labelStep(n: number): number {
  return n <= 8 ? 1 : Math.ceil(n / 6);
}

// 저밀도 상한 9→10 근거(실측 d63 라이브 리포트): dscan 은 시간 히스토그램(데이터
// 온도)을 9버킷, **파일 크기 히스토그램을 10버킷**으로 낸다. 9 였을 때 크기
// 분포만 고밀도로 떨어져 값 라벨과 누적 오버레이가 통째로 사라지고 축 라벨은
// 절반만 남았는데(labelStep(10)=2), 캡션은 누적선을 설명하고 있어 화면이
// 거짓말을 했다. 기존 소비자 영향: 잡 통계 분포 4종(5버킷+overflow=6)은 원래
// 저밀도, 처리량 24h(≤24버킷)·7d(≤7버킷)는 각각 고밀도·저밀도 그대로다 --
// 다만 처리량은 데이터가 있는 버킷만 오므로 마침 10개인 창은 저밀도로 바뀐다
// (값·전 라벨이 더 보이는 방향이라 안전하다).
const SPARSE_MAX = 10;       // 이하면 저밀도 모드(값 직표기 가능한 밀도)
