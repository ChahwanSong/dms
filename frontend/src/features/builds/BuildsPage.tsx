...
// 콘텐츠 컬럼 폭. 전폭을 쓰지 않고 가운데로 모은다 -- 제출 폼은 "한 번에 하나씩
// 정하는" 화면이라 좁을수록 읽기 쉽다.
const COLUMN = "mx-auto w-full max-w-3xl";

// 목록만 한 단계 넓은 컬럼을 쓴다(같은 중심축, 여백만 줄어듦).
//
// 폼과 같은 3xl 에 넣어 봤더니 8열이 컨테이너에 눌려 시각·ref·이미지가 2~4줄로
// 접히고 행 높이가 들쭉날쭉해졌다(실측) -- 밀도를 낮추려다 사유 3줄 문제를 열
// 세 개로 옮겨 놓은 꼴이었다. 표는 좁힌다고 정보가 줄지 않고 접히기만 한다.
const LIST_COLUMN = "mx-auto w-full max-w-5xl";
...
export function BuildsPage() {
  ...
  // 「취소」는 화면을 떠나지 않고 폼만 초기화한다 -- 이 화면은 마법사가 아니라
  // "폼 + 이력"이 한 페이지에 있는 형태라, 다른 경로로 튕기면 방금 보던 목록까지
  // 잃는다. 지난 제출 오류도 함께 지운다(낡은 409 가 남지 않게).
  const cancel = () => {
  ...
  return (
    <section className="space-y-6">
      ...
      {q.isLoading ? (
        <p className={`${LIST_COLUMN} text-muted`}>불러오는 중…</p>
      ) : q.isError ? (
        <p className={`${LIST_COLUMN} text-bad`}>{(q.error as ApiError).message}</p>
      ) : (
        <Card className={LIST_COLUMN}>
          ...
          <Table>
            <thead>
              {/* commit·노드는 상세로 밀었다: commit 은 실패 시 대개 —이고, 노드는
                  위 확인 박스에 이미 있어 매 행 반복하면 밀도만 올린다. 열 8개는
                  e2e L2 의 셀 하한(minTableCells: 8)이기도 하다. */}
