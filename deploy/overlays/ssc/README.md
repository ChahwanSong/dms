...
가 hostNetwork 로 호스트 :80/:443 를 직접 리슨한다. 사용자는 `https://<public IP>`
로 바로 포탈에 닿는다.
...
# 2) ingress-nginx 를 hostNetwork 로 설치(웹 노드 고정) — 애드온, 오버레이 밖
...
# 3) 값 채우기 — 파일 하나 (PORTAL_VIP 없음; WEB_NODE·PORTAL_PUBLIC_IP 추가)
cp deploy/overlays/ssc/values.env.example deploy/overlays/ssc/values.env
$EDITOR deploy/overlays/ssc/values.env
...
| 값 | PORTAL_VIP + PORTAL_DOMAIN | PORTAL_PUBLIC_IP + WEB_NODE + PORTAL_DOMAIN |
