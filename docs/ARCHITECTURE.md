...
- preflight 마커 사유는 PREFLIGHT_REASONS 화이트리스트 전수(execution_manifests.py:323 + parse_preflight_reason:329)와 스크립트 실물이 일치해야 한다 — 계약 테스트가 스크립트에서 마커를 추출해 대조한다. 다섯 코드 철자는 frontend reasonCodes.json·api.ts REASON_MESSAGES와 동일(build 프로브 화이트리스트와 같은 규약).
...
5. **<base> 와 <base>/<job_id> 는 요청자 쓰기 불가**(root:root·비-world-writable; 러너는 <job_id> 를 root 로 만들고 <phase> 만 chown). 깨지면 assert_contained 의 기준(realpath(<base>/<job_id>))을 요청자가 심링크로 옮길 수 있다. base 의 **부모(공용 디렉터리)는 root:root 770 까지 잠글 수 있다** — 잡 파드는 base 를 전용 볼륨(artifact_base.ARTIFACT_MOUNT, execution_volcano._volumes)으로 받아 요청자 uid 도구가 그 부모를 통과하지 않는다(d129). 사용자 데이터 root(storages.managed_root)는 공용 디렉터리 아래에 두지 않는다 — preflight 가 요청자 uid 로 검사한다.
