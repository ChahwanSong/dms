...
Last updated: 2026-05-28 07:34 +0900
...
- Phase 3까지의 실제 live 검증 대상은 PostgreSQL, OpenLDAP, Kubernetes read-only inventory다.
...
다음 항목은 Phase 3까지 완료된 기능으로 보지 않는다.
...
- 실제 Kubernetes `ResourceQuota` create/update/delete/sync
- 실제 PVC admission success/failure를 통한 quota 검증
...
- 다음 phase는 `cluster-b/testbed-longhorn`을 사용한 Kubernetes namespace storage quota live adapter가 적절하다.
