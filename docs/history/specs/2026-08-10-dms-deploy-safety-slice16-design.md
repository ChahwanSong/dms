...
`/sys/devices/virtual/net/` 이 존재하지만, 거기 든 것은 **파드 netns 의** 가상
인터페이스다. 그래서 기본값을 `/sys/devices/virtual/net` 으로 두면 마운트가 없는
배포에서 **다른 네임스페이스의 집합으로 호스트의 인터페이스 목록을 거르게** 된다 —
애초에 비교 대상이 아닌 두 집합이라, **이름이 겹치는 호스트 인터페이스는 무엇이든**
가상으로 오판돼 제외된다. 정확히 고치려던 것보다 더 나쁜 값이 된다.

`eth0` 은 그 충돌이 가장 흔하게 일어나는 사례일 뿐이다 — CNI 가 파드 인터페이스를
관례적으로 `eth0` 으로 만들고, VM·클라우드 호스트의 물리 NIC 도 흔히 `eth0` 이라
양쪽에서 동시에 잘 나온다. 물리 NIC 가 `ens192`/`enp5s0`/`bond0`/`eno1` 인 환경에서도
파드 쪽에 같은 이름이 있으면 **동일하게 터진다**. (이 문단은 원래 "파드 인터페이스는
보통 `eth0`" 을 전제로 쓰여 있었다. 이름 단정은 환경 의존이고 — 판별 규칙 자체는
이름을 보지 않으므로 — 위험의 정체를 네임스페이스 교차 이름 충돌로 다시 적었다.)

따라서 `DMS_AGENT_VIRTUAL_NET_PATH` 는 **기본 미설정**이고, 명시적으로 설정됐고 읽히는
