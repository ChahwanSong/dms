...
- filesystem import는 payload passthrough로 `expires_at`이 DB desired state에 남을 수는 있지만, import 전용 alias/default/future timestamp validation과 marker 반영 규칙이 정의되어 있지 않다.
- Kubernetes namespace quota resource는 create/update/reset payload에 `expires_at` 같은 운영 metadata를 보존할 여지는 있지만, timestamp validation, expired query, action-required, sweep side effect가 없다.
...
- filesystem resource update request에서 expiry timestamp를 optional하게 변경하거나 명시적으로 제거할 수 있다.
- Kubernetes namespace quota create/update/default-reset request에서 expiry timestamp를 설정/변경하거나 명시적으로 제거할 수 있다.
- filesystem import request와 Kubernetes namespace quota import/adoption request는 optional `expires_at`/`expiry_at`을 받는다.
- import request에 expiry timestamp가 있으면 그 값을 사용하고, 없으면 planner 기준 server-side now부터 365일 뒤 값을 canonical `expires_at`으로 설정한다.
- import request는 finite expiry를 반드시 갖는다. 따라서 import에서 `clear_expires_at=true`는 backend side effect 없이 `Rejected`로 종료한다.
- DB에는 기존 filesystem create semantics와 일관되게 canonical field `expires_at`으로 저장한다.
- API는 사용자가 말한 `expiry_at` alias도 받을 수 있지만, 내부 저장과 response는 `expires_at`으로 normalize한다.
- `expires_at`과 `expiry_at`이 동시에 들어오고 값이 다르면 backend side effect 없이 `Rejected`로 종료한다.
- `clear_expires_at=true`와 `expires_at`/`expiry_at`이 동시에 들어오면 backend side effect 없이 `Rejected`로 종료한다.
- expiry timestamp는 timezone이 있는 ISO-8601 timestamp로 검증한다. `Z` suffix와 timezone offset을 허용한다.
- create/update/reset 같은 expiry write request에서 expiry timestamp가 planner 기준 server-side now보다 과거이거나 같으면 backend side effect 없이 `Rejected`로 종료한다.
...
기존 filesystem Resource Management는 `expires_at`을 사용한다. Phase 15도 DB, desired state, applied state, observed state, query response의 canonical field는 `expires_at`으로 통일한다.

다만 사용자/포털 요청에서 `expiry_at`을 보내는 경우가 있을 수 있으므로 API request payload에서는 alias로 허용할 수 있다.

권장 normalization 규칙:

```text
payload.expires_at present only  -> desired_state.expires_at
payload.expiry_at present only   -> desired_state.expires_at
both present and equal           -> desired_state.expires_at
both present and different       -> Rejected, backend_side_effect=false
clear_expires_at=true            -> remove desired_state.expires_at
clear_expires_at + expiry value  -> Rejected, backend_side_effect=false
expires_at=null without clear    -> Rejected, backend_side_effect=false
expiry_at=null without clear     -> Rejected, backend_side_effect=false
```
...
- filesystem import와 Kubernetes namespace quota import/adoption은 `expires_at` 또는 `expiry_at`이 없으면 server-side now 기준 365일 뒤 timestamp를 생성한다.
...
- import request에 `expires_at` 또는 `expiry_at`이 있으면 default를 적용하지 않고 요청값을 검증/normalize한다.
- import request의 요청값도 현재 시각보다 과거이거나 같으면 reject한다.
- import request에서 `clear_expires_at=true`는 허용하지 않는다. import는 full managed lifecycle에 편입되는 시점이므로 finite expiry가 필요하다.
...
```text
expires_at_clear_unsupported_for_import
expires_at_default_failed
```

## Filesystem Resource Expiry Update
...
```json
{
  ...
  "payload": {
    ...
    "reason": "remove expiry after approval"
  }
}
```
...
- update payload에 `expires_at`/`expiry_at`/`clear_expires_at`이 없으면 기존 expiry metadata를 보존한다.
...
- `clear_expires_at=true`이면 desired/applied state와 marker metadata에서 expiry를 제거한다.
- expired-unblocked filesystem action-required는 expiry가 미래로 연장되면 사라져야 한다.

구현 힌트:

- `PHASE12_FILESYSTEM_UPDATE_ALLOWED_PAYLOAD_FIELDS`에 `expires_at`, `expiry_at`, `clear_expires_at`을 추가한다.
...
```json
{
  ...
  "payload": {
    ...
    "expiry_at": "2027-06-01T00:00:00Z",
...
- `expires_at`/`expiry_at`이 있으면 shared expiry validation을 적용한다.
- `expires_at`/`expiry_at`이 없으면 server-side now + 365일을 `expires_at`으로 설정한다.
- import payload의 `expires_at`/`expiry_at`이 과거 또는 현재 timestamp이면 reject한다.
- import payload의 `clear_expires_at=true`는 reject한다.
...
```json
{
  ...
  "payload": {
    ...
    "expiry_at": "2026-07-01T00:00:00Z"
  }
}
```

두 요청 모두 persisted desired state에는 다음처럼 저장돼야 한다.
...
- update payload에 `expires_at`/`expiry_at`/`clear_expires_at`이 없으면 기존 expiry metadata를 보존한다.
- `reset_quota_to_default=true` update에서도 expiry metadata update/clear를 허용한다.
...
- `clear_expires_at=true`이면 default reset과 동시에 expiry를 제거한다.
...
- helper 책임:
  - `expires_at`/`expiry_at` alias normalization
  - conflicting aliases reject
  - `clear_expires_at` conflict reject
...
- filesystem update allowed payload fields에 `expires_at`, `expiry_at`, `clear_expires_at`을 추가한다.
- filesystem import allowed payload fields에 `expires_at`, `expiry_at`을 추가하고 `clear_expires_at`은 explicit reject한다.
- Kubernetes quota create/update/default-reset/import validation에 shared expiry validation을 적용한다.
- Kubernetes quota `expiry_at` alias normalization을 구현한다.
- clear request에서는 기존 desired state에서 `expires_at`을 제거한다.
- update payload에 expiry 관련 field가 없으면 기존 desired state를 보존한다.
...
- filesystem marker metadata에 `expires_at` create/import/update/clear를 반영한다.
...
1. host-mounted CephFS filesystem resource를 생성한 뒤 update request로 미래 `expires_at`을 설정한다.
2. 같은 filesystem resource에 과거 `expires_at` update를 보내면 `Rejected`되고 backend side effect가 없는지 확인한다.
3. filesystem import request에 미래 `expiry_at` alias를 넣으면 canonical `expires_at`으로 저장되고 marker에 반영되는지 확인한다.
4. filesystem import request에서 expiry를 생략하면 server-side now + 365일 default `expires_at`이 저장되는지 확인한다.
5. filesystem import request에서 과거 `expires_at`이나 `clear_expires_at=true`를 보내면 `Rejected`되고 marker/quota side effect가 없는지 확인한다.
6. expired filesystem resource의 expiry를 미래로 연장하면 `filesystem_expired_unblocked` action-required issue가 사라지는지 확인한다. 이 expired fixture는 repository fixture 또는 짧은 미래 expiry 후 대기 방식으로 만든다.
7. `cluster-b/testbed-longhorn` storage mapping으로 namespace quota를 생성하면서 미래 `expiry_at` alias를 넣는다.
8. DB desired state와 query response에는 canonical `expires_at`으로 저장/반환되는지 확인한다.
9. Kubernetes quota 과거 `expires_at` create/update request가 `Rejected`되고 live `ResourceQuota`를 만들거나 변경하지 않는지 확인한다.
10. Kubernetes namespace quota import request에 미래 `expiry_at` alias를 넣으면 canonical `expires_at`으로 저장되고 live hard는 변경되지 않는지 확인한다.
11. Kubernetes namespace quota import request에서 expiry를 생략하면 server-side now + 365일 default `expires_at`이 저장되는지 확인한다.
12. Kubernetes namespace quota import request에서 과거 `expires_at`이나 `clear_expires_at=true`를 보내면 `Rejected`되고 live `ResourceQuota`가 변경되지 않는지 확인한다.
13. future `expires_at` resource는 `status=expired` query와 action-required에 나타나지 않는다.
14. 짧은 미래 expiry를 설정하고 만료될 때까지 bounded wait한 뒤 expired query API가 target을 반환하는지 확인한다.
15. expired but unblocked 상태가 action-required에 `kubernetes_quota_expired_unblocked`로 올라간다.
16. `dry_run=true` expiration sweep은 target만 기록하고 live ResourceQuota hard를 바꾸지 않는다.
17. `dry_run=false` expiration sweep은 live `ResourceQuota/dms-storage-quota` hard를 zero로 만든다.
18. block 후 action-required에서 expired-unblocked issue가 사라진다.
19. unblock request가 기존 hard limit을 복구한다.
20. `resource_type=system` expired quota는 sweep에서 skip되고 skip reason이 result/action-required에 남는다.
21. namespace cleanup은 테스트 cleanup으로만 수행한다. DMS lifecycle namespace delete를 구현한 것으로 보지 않는다.
...
- filesystem update payload의 `expiry_at` alias가 `expires_at`으로 normalize된다.
- filesystem update payload의 과거/current `expires_at`은 `Rejected`되고 plan이 생성되지 않는다.
- filesystem update payload의 `clear_expires_at=true`가 expiry metadata를 제거한다.
- filesystem import payload의 `expires_at`이 desired state와 marker metadata에 저장된다.
- filesystem import payload의 `expiry_at` alias가 `expires_at`으로 normalize된다.
- filesystem import payload의 missing expiry가 server-side now + 365일 default로 채워진다.
- filesystem import payload의 과거/current `expires_at`과 `clear_expires_at=true`는 `Rejected`되고 plan이 생성되지 않는다.
...
- Kubernetes create payload의 `expiry_at` alias가 `expires_at`으로 normalize된다.
- Kubernetes update payload의 `expires_at`이 기존 desired state에 merge된다.
- Kubernetes `clear_expires_at=true`가 expiry metadata를 제거한다.
- Kubernetes import payload의 `expires_at`이 desired state와 observed metadata evidence에 저장된다.
- Kubernetes import payload의 `expiry_at` alias가 `expires_at`으로 normalize된다.
- Kubernetes import payload의 missing expiry가 server-side now + 365일 default로 채워진다.
- Kubernetes import payload의 과거/current `expires_at`과 `clear_expires_at=true`는 `Rejected`되고 plan이 생성되지 않는다.
- Kubernetes import는 live `ResourceQuota.spec.hard`를 바꾸지 않고 DB desired/applied/observed state를 bootstrap한다.
- `expires_at`과 `expiry_at` 값이 다르면 request가 `Rejected`되고 plan이 생성되지 않는다.
