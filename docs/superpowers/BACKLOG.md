...
갱신: 2026-08-10. 기준 커밋: `main` = `db8f482` (슬라이스 17 까지 병합됨).

---

## 0. 현재 상태

- 슬라이스 1~17 완료. 슬라이스 18·19·20 은 설계 확정, 구현 착수.
- 테스트베드 이미지: 제어면 `dms:d28`(슬라이스 17), 에이전트 `dms-agent:d27`,
...
### 슬라이스 19 «계정 위생» (사용자 요청, 일부 제외) — 설계 확정
- 설계: `specs/2026-08-10-dms-account-hygiene-slice19-design.md`.
- 사용자 확정: 계정은 **하드 삭제**(이력 문자열 actor 는 보존). 토큰 경로의
  `x-dms-actor` 는 **`node:<이름>` 전용**으로 좁히고 특권 승격은 **세션에서만** 허용.
- 아래는 착수 당시의 원 항목이다(기록 보존용).
...
### 슬라이스 20 «Volcano 대기 이력» (슬라이스 17 파생) — 설계 확정
- 설계: `specs/2026-08-10-dms-sched-wait-slice20-design.md`.
- 끝난 잡의 스케줄링 대기가 사후에 없다 — PodGroup 이 잡 종료와 함께 삭제되기 때문.
- **원안(컨트롤러가 PodGroup 샘플링)을 기각했다**: PodGroup 이름 `<vcjob>-<uid>` 는
  계약이 아니고 DMS 라벨도 없으며, 컨트롤러 무권한이 `test_rbac_contract.py:92-99` 에
  계약으로 고정돼 있다. 대신 **스테퍼가 이미 매 틱 하고 있는 vcjob phase 관측**을
  써서 `data_jobs.sched_wait_seconds`(execution vcjob 제출 → 첫 Running 관측)를
  남긴다 → 추가 k8s 호출 0, RBAC 변경 0.
