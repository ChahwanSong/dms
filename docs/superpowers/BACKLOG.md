...
**할 일(다음 슬라이스)**: psycopg 연결에 **유한 `connect_timeout`**(예: 5초)을 주어
재연결이 빠르게 실패하게 하고, readyz 가 제때 503 을 내 카운터가 증가하게 한다.
그러면 자기 종료가 설계대로 발화한다. 단위 테스트는 카운터·리셋·비활성을 이미
고정했으므로 남은 것은 **연결 타임아웃 + 라이브 발화 재실증**이다.
