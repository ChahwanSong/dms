...
- 슬라이스 1~15 완료. `main`에 병합됨(로컬). **origin push는 사용자 몫 — 미푸시.**
- 테스트베드: 잡 이미지 `dms-mpifileutils:job4`, 제어면 `dms:d25`.
- nsync 카운트 파서(`8582d6d`)는 **코드만 완료** — `job5` 재빌드 + 실증 필요.
...
- **태그 동기화가 4파일 수기** — 템플릿 계층 없음(의도적, `deploy/README.md`).
