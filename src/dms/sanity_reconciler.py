...
def build_sanity_service(
    ...
    """Construct the sanity service outside the API (kubernetes_inventory is optional;
    DM/filesystem readiness is derived from agent reports, the same default the app uses
    when no Kubernetes inventory adapter is configured)."""
    return StorageMappingSanityService(
        repository=repository,
        inventory_service=EffectiveInventoryService(
            repository=repository,
            kubernetes_inventory=None,
            settings=settings,
        ),
        settings=settings,
    )
