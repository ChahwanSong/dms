...
import { backendType, isForPv } from "../storage/helpers";
...
// The assembled/typed string is emitted via onPath and sent verbatim as src_path/dst_path
// (DMS treats it as managed_root-relative). Remount per storage (key=storage_name in the
// parent) so switching storages resets the sub-fields.
export default function EndpointPath({
  ...
  return (
    <div className="pv-path">
      ...
      <div className="pv-path-preview">
        <span className="muted small">→ 경로 (managed_root 상대)</span>
        <code className={path ? undefined : "muted"}>
          {path || (kind === "ceph" ? "csi/<subvol>/<uuid>" : "pvc-<uid>/pvc-<uid>-data")}
        </code>
      </div>
    </div>
  );
