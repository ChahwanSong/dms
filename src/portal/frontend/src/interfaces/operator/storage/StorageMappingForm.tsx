      ...
      <div className="modal wide" onClick={(e) => e.stopPropagation()}>
        ...
        <form className="form" onSubmit={submit}>
          <div className="banner info">
            {mode === "edit" ? (
              <>
                <strong>저장하면 변경사항이 즉시 적용됩니다.</strong> 입력한 전체 설정으로
                교체(부분 수정 아님)하고 sanity를 재검사합니다.
              </>
            ) : (
              <>
                <strong>생성하면 매핑이 등록되고 sanity가 검사됩니다.</strong>
              </>
            )}
            <ul>
              <li>
                아래 <strong>필드 설명</strong> 표의 <code>{currentBackend}</code> 필드만 실제
                효과가 있습니다 — <strong>표에 없는 키는 저장돼도 무시됩니다.</strong>
              </li>
              {mode === "edit" && (
                <>
                  <li>
                  ...
                  <li>
                    해당 스토리지에 <strong>진행 중인 작업</strong>이 있으면 수정이
                    거부됩니다(409). 작업이 끝난 뒤 다시 시도하세요.
                  </li>
                  <li>
          ...
          {fieldDocs.length > 0 && (
            <div className="field-ref">
              <div className="field-ref-head">
                ...
                <span className="muted small"> · 이 표에 없는 키는 저장돼도 무시됩니다</span>
              </div>
